...
public class GstSequenceController {
	...
	public void setSequenceData(ActionRequest request, ActionResponse response) {
		...
		Sequence isSequence=sequenceRepo.find(sequence.getId());
		if (isSequence == null) {
	...
	public void setPartySequence(ActionRequest request, ActionResponse response) {
		...
		if (party.getReference() == null) {
			sequence = Beans.get(SequenceRepository.class).all().filter("self.model.name = ?", "Party").fetchOne();
			if (sequence != null) {
				...
				int paddingvalue = Integer.parseInt(
						(sequence.getNextNumber().substring(prefix.length(), prefix.length() + sequence.getPadding())))
						+ 1;
	...
	public void setInvoiceSequence(ActionRequest request, ActionResponse response) {
		...
		if (invoice.getReference() == null) {
			sequence = Beans.get(SequenceRepository.class).all().filter("self.model.name = ?", "Invoice").fetchOne();
			if (sequence != null) {
